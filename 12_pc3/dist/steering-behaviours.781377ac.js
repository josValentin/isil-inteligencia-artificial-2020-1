...
parcelRequire = (function (modules, cache, entry, globalName) {
  ...
  return newRequire;
})({"src/steering-behaviours.js":[function(require,module,exports) {
function _classCallCheck(instance, Constructor) { if (!(instance instanceof Constructor)) { throw new TypeError("Cannot call a class as a function"); } }

var SteeringBehaviours = function SteeringBehaviours() {
  _classCallCheck(this, SteeringBehaviours);
};

SteeringBehaviours.seek = function (agent, target, radius) {
  var distance = dist(agent.position.x, agent.position.y, target.x, target.y);
...
SteeringBehaviours.flee = function (agent, target, radius) {
  var distance = agent.position.copy().sub(target).mag();
  var desired = createVector(0, 0);
...
SteeringBehaviours.arrive = function (agent, target, radiusArrive, radius) {
  var desired;
  var distance = target.copy().sub(agent.position).mag();

  if (distance < radius) {
    if (distance > radiusArrive) {
      desired = target.copy().sub(agent.position).normalize().mult(agent.MAX_SPEED);
    } else {
      var speed = map(distance, radiusArrive, 0, agent.MAX_SPEED, 0);
...
SteeringBehaviours.separate = function (agent, others, separateRadius) {
  ...
  for (var i = 0; i < Global.npcs.length; i++) {
    var npc = Global.npcs[i];
    if (agent == npc) continue;
    var distance = agent.position.copy().sub(npc.position).mag(); // verificar si estoy dentro del entorno

    if (distance == 0) {
      desired.add(createVector(random(0.1), random(0.1)));
    } else if (distance < separateRadius) {
      // sumar todos los deseados de los flees
      var desiredAux = SteeringBehaviours.flee(agent, npc.position, 9999999);
...
},{}]},{},["../../../../.config/yarn/global/node_modules/parcel-bundler/src/builtins/hmr-runtime.js","src/steering-behaviours.js"], null)
//# sourceMappingURL=/steering-behaviours.781377ac.js.map